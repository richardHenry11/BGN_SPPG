// lib/screens/pengiriman/tab_keluar.dart
...
class TabKeluar extends StatefulWidget {
  const TabKeluar({super.key});

  @override
  State<TabKeluar> createState() => _TabKeluarState();
}

class _TabKeluarState extends State<TabKeluar> {
  int? _selectedId;

  @override
  Widget build(BuildContext context) {
    final provider = context.watch<PengirimanProvider>();
    final belumKeluar = provider.pengirimanList
        .where((p) => p.validasiKeluar == null)
        .toList();

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Validasi barang keluar',
            style: TextStyle(
                fontSize: 13,
        ...
        if (belumKeluar.isEmpty)
          _emptyState('Semua barang sudah keluar')
        else
          ...belumKeluar.map((item) {
                        ...
                        children: [
                          Text(item.alamat,
                              style: const TextStyle(
                                  ...
                                  color: BGNColors.textPrimary)),
                          Text(
                              '${item.waktu} · ${item.porsiRencana} porsi',
                              style: const TextStyle(
                                  fontSize: 10,
...
class _FormValidasiKeluar extends StatefulWidget {
  final int pengirimanId;
  final VoidCallback onBerhasil;
  const _FormValidasiKeluar(
      {required this.pengirimanId, required this.onBerhasil});

  @override
  State<_FormValidasiKeluar> createState() => _FormValidasiKeluarState();
}

class _FormValidasiKeluarState extends State<_FormValidasiKeluar> {
  ...
  void _handleKonfirmasi() {
    ...
    if (jumlah == null) {
      setState(() => _errorMsg = 'Jumlah barang keluar wajib diisi');
      return;
    }
    ...
    context.read<PengirimanProvider>().inputValidasiKeluar(
          widget.pengirimanId,
          ValidasiData(
            jumlah: jumlah,
            kondisi: _kondisi,
  ...
  @override
  Widget build(BuildContext context) {
    final provider = context.read<PengirimanProvider>();
    final pengiriman = provider.getPengirimanById(widget.pengirimanId);
    if (pengiriman == null) return const SizedBox();

    final jumlahInput = int.tryParse(_jumlahCtrl.text);
    final adaSelisih = jumlahInput != null &&
        jumlahInput != pengiriman.porsiRencana;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(TablerIcons.arrow_up,
          ...
          // Jumlah
          const Text('Jumlah barang keluar (porsi)',
              style:
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Rencana: ${pengiriman.porsiRencana}',
              ...
              child: Row(
                children: [
                  const Icon(TablerIcons.alert_triangle,
                      size: 12, color: BGNColors.warning),
                  const SizedBox(width: 4),
                  Text(
                    'Selisih ${(jumlahInput - pengiriman.porsiRencana).abs()} porsi dari rencana',
                    style: const TextStyle(
                        fontSize: 10, color: BGNColors.warning),
                  ),
                ],
              ),
            ),
            ...
            children: _kondisiList.map((k) {
              ...
              return Expanded(
                child: GestureDetector(
                  onTap: () => setState(() => _kondisi = k['id'] as String),
                  child: Container(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              icon: const Icon(TablerIcons.arrow_up, size: 16),
              label: const Text('Konfirmasi barang keluar'),
            ),
          ),
        ],
      ),
